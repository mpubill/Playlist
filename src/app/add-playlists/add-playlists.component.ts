...
@Component({
  selector: 'app-add-playlists',
  templateUrl: './add-playlists.component.html',
  styleUrls: ['./add-playlists.component.css']
})
export class AddPlaylistsComponent {
  ...
  mostrarAlerta() {
    Swal.fire({
      title: 'PlayList creada Correctamente',
  ...
  GuardarPlayList(){
    ...
    this.http.post<any>('http://localhost:3000/subir-imagen', formData).subscribe(
      (respuesta) => {
        ...
        this.PlaylistService.agregarPlaylist(user).subscribe(
          (data: any) => {
